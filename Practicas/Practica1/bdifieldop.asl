...
+target_reached(T): going_to_give_ammo
  <- 
  -going_to_give_ammo;
  -target_reached(T);
  -going(T);
...
+enemies_in_fov(ID,Type,Angle,Distance,Health,Position): team(200) & not going_to_give_ammo & not going_to_heal & not chasing
  <-
  .shoot(8, Position);
  .look_at(Position);
  !go_to(Position);
...
+enemies_in_fov(ID,Type,Angle,Distance,Health,Position): team(200) & not going_to_give_ammo & not going_to_heal & chased(ID)
  <-
  .shoot(8, Position);
  .look_at(Position);
  !go_to(Position);
